...
pub fn PrincipalEdit() -> impl IntoView {
    ...
    let forwarding_target = create_rw_signal(String::new());
    let forwarding_original = create_rw_signal(String::new());
    let sieve_script = create_rw_signal(String::new());
    ...
    let save_changes = create_action(move |changes: &Principal| {
        ...
        let auth = auth.get();
        let selected_type = selected_type.get();
        let fwd_target = forwarding_target.get_untracked();
        let fwd_original = forwarding_original.get_untracked();
        let fwd_script = sieve_script.get_untracked();
        let fwd_email = current.emails.as_str().unwrap_or_default().to_string();
        ...
        async move {
            ...
            if result.is_ok()
                ...
                && fwd_target != fwd_original
                && !fwd_email.is_empty()
            {
                let new_script = update_forwarding_script(&fwd_script, &fwd_email, &fwd_target);
                let updates = vec![UpdateSettings::Insert {
                    prefix: None,
                    values: vec![(
                        "sieve.trusted.scripts.forward-to-gmail.contents".to_string(),
                        new_script,
                    )],
                    assert_empty: false,
                }];
                if let Ok(req) = HttpRequest::post("/api/settings")
                    .with_authorization(&auth)
                    .with_body(updates)
                {
                    let _ = req.send::<Option<String>>().await;
                    let _ = HttpRequest::get("/api/reload/sieve")
                        .with_authorization(&auth)
                        .send::<serde_json::Value>()
                        .await;
                }
            ...
            <Transition fallback=Skeleton set_pending>
                ...
                {move || match fetch_principal.get() {
                    ...
                    Some(Ok(principal)) => {
                        ...
                        current_principal.set(principal.clone());
                        let typ = selected_type.get();

                        if matches!(typ, PrincipalType::Individual) {
                            if let Some(email) = principal.emails.as_str() {
                                let email = email.to_string();
                                let auth_fetch = auth.get_untracked();
                                spawn_local(async move {
                                    if let Ok(settings) = HttpRequest::get("/api/settings/keys")
                                        .with_authorization(&auth_fetch)
                                        .with_parameter("keys", "sieve.trusted.scripts.forward-to-gmail.contents")
                                        .send::<AHashMap<String, Option<String>>>()
                                        .await
                                    {
                                        for value in settings.values() {
                                            if let Some(script) = value {
                                                sieve_script.set(script.clone());
                                                if let Some(target) = parse_forwarding_target(script, &email) {
                                                    forwarding_target.set(target.clone());
                                                    forwarding_original.set(target);
                                                }
                                                break;
                                            }
                                        }
                                    ...
                                    <FormSection stacked=true>
                                        ...
                                        >
                                            <input
                                                ...
                                                prop:value=move || forwarding_target.get()
                                                on:input=move |ev| {
                                                    forwarding_target.set(event_target_value(&ev));
                                                }
                                            />
                                            <p class="mt-2 text-sm text-gray-500 dark:text-neutral-400">
                                                "Enter an email address to forward all incoming mail. Leave empty to disable forwarding."
                                            </p>
                                        </FormItem>
                                    </FormSection>
...
fn parse_forwarding_target(script: &str, email: &str) -> Option<String> {
    let needle = format!("if envelope :is \"to\" \"{}\"", email);
    let block_start = script.find(&needle)?;
    let rest = &script[block_start..];
    let redirect_prefix = "redirect \"";
    let redirect_pos = rest.find(redirect_prefix)?;
    let target_start = redirect_pos + redirect_prefix.len();
    let target_end = rest[target_start..].find('"')? + target_start;
    Some(rest[target_start..target_end].to_string())
}

fn update_forwarding_script(script: &str, email: &str, target: &str) -> String {
    let needle = format!("if envelope :is \"to\" \"{}\"", email);

    if let Some(block_start) = script.find(&needle) {
        let rest = &script[block_start..];
        if let Some(close_brace) = rest.find('}') {
            let mut block_end = block_start + close_brace + 1;
            if script.len() > block_end && script.as_bytes()[block_end] == b'\n' {
                block_end += 1;
            }

            let before = &script[..block_start];
            let after = &script[block_end..];
